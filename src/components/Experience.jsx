...
export const Experience = () => {
  ...
  const addSignLanguageMessage = useAITeacher((state) => state.addSignLanguageMessage);

  
  const teacherConfig = modelConfig.teachers[teacher];
  const classroomConfig = modelConfig.classrooms[classroom];

  const handleSignLanguageResult = (text) => {
    console.log("Received sign language result:", text);
    addSignLanguageMessage(text);
  };

  return (
    <>
      <CVCamera onSignLanguageResult={handleSignLanguageResult} />
